...
async def main():
    sites = [
        ('yoga','https://www.yogaworks.com'),
        ('yoga','https://www.corepoweryoga.com'),
        ('physio','https://www.athletico.com'),
        ('gym','https://www.barrys.com'),
    ...
    for niche, url in sites:
        ...
        try:
            ...
            print(f'  {d["title"][:60]}')
            print(f'  Font: {d["font"]}')
            print(f'  H1: {d["h1Size"]}/{d["h1Weight"]} "{d["h1Text"][:50]}"')
            print(f'  H1 color: {d["h1Color"]}')
            for s in d.get('sections',[])[:3]:
                print(f'  Sec: bg={s["bg"]} text={s["color"]}')
            for b in d.get('buttons',[])[:2]:
                print(f'  Btn: "{b["text"][:25]}" bg={b["bg"]} radius={b["radius"]} size={b["fontSize"]}')
        except Exception as e:
            print(f'  Error: {e}')
    
    # Merge with existing data
    try:
        with open('/home/prata/leads/design-inspiration/design_tokens.json') as f:
    ...
    print(f'\nTotal saved: {len(existing)} analyses')
